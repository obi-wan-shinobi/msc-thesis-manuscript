...
operator as described in equation~\ref{eq:bg-convolution-operator} becomes
\[
	(T_{p}g)(\theta)
	:=
	\int_0^{2\pi}
...
\subsection{Preliminary experiments across architectures}
...
\begin{figure}[!htbp]
	...
	\caption{Preliminary experiments across architectures trained at finite width on
		...
		against training step on a logarithmic scale. Across all three architectures
		the lowest-frequency mode \(\cos\gamma\) is driven down first and fastest,
		with the higher-frequency modes decaying later, though the ordering is not
		strict and is least clean for the attention block. The sharp downward spikes
		(for example \(\cos 2\gamma\) in the MLPs) are sign changes of the residual
		coefficient, which appear as cusps on a logarithmic amplitude axis.}
