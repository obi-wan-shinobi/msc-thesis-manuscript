...
together. The mode-wise residual plots show which modes present in the target are
...
subspaces. The spectral-strength plots measure the average eigenvalue
inside each Fourier block, equivalently measuring the operator strength in that Fourier subspace.

If the evolving kernel helps because it preserves Fourier geometry better, then one would expect improved alignment.
...
show three recurring trends. First, the mode-wise residual plots show that the evolving dynamics gives
smaller residuals for several low-frequency target modes at widths \(128\) and
\(256\). At width \(512\), this difference is smaller. At width \(1024\), the
frozen dynamics is comparable or better on the modes that dominate the final
loss.

Second, the subspace cosine similarity plots show that kernel evolution does
not increase alignment with the Fourier subspaces. Several higher-frequency
subspaces lose alignment during training.

Third, the spectral-strength plots show that the evolving kernel increases the
average operator strength in the lowest Fourier blocks, especially \(k=0\) and
...
Thus, in these experiments, the evolving tangent kernel changes more at small
width than at large width. This is consistent with the stronger difference
between frozen and evolving loss curves at widths \(128\) and \(256\).
...
Figures~\ref{fig:evolving-joint-w128}--\ref{fig:evolving-joint-w1024} show that
this drift is accompanied by two visible changes: increased low-frequency
spectral strength and reduced alignment for some Fourier subspaces.
...
The effect also decreases with width. At width \(128\), the evolving curves move
well above the frozen baseline. At width \(1024\), the same ordering is still
visible, but the increase is smaller. These experiments show that the
low-frequency strength increase persists across the three amplitude choices.
